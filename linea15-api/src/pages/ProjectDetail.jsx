...
import { ROLES, ROLE_COLORS } from './Users.jsx';

function hasPerm(role, perm) { return ROLES[role]?.perms?.includes(perm) ?? false; }
...
function validateFinalize(controls, evs) {
  ...
  if (incomplete > 0) return `Faltan ${incomplete} controles por documentar antes de finalizar.`;
  if (!evs || evs.length === 0) return 'Debes agregar al menos una evidencia antes de finalizar.';
  return null;
}

// ── Input compliance con voz + autoguardado ──────────────────────────
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <div style={{ display: 'flex', gap: 5 }}>
        <textarea className="form-control" rows={2} placeholder="Documentar cumplimiento…" value={value}
          onChange={e => setValue(e.target.value)}
          style={{ fontSize: '.78rem', flex: 1, minHeight: 'unset', borderColor: listening ? 'var(--danger)' : undefined }} />
        <button className={`btn btn-sm ${listening ? 'btn-danger' : 'btn-secondary'}`} onClick={toggleVoice}
          title={listening ? 'Detener dictado' : 'Dictado por voz'} style={{ flexShrink: 0, padding: '6px 8px' }}>
          {listening
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <button onClick={() => setOpen(o=>!o)} style={{ display:'flex', alignItems:'center', gap:8, width:'100%', padding:'8px 12px', background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'var(--radius-sm)', cursor:'pointer', color:'var(--text-muted)', fontSize:'.72rem', fontWeight:700, fontFamily:'var(--font)', textTransform:'uppercase', letterSpacing:'.5px' }}>
        ...
        ⚖️ Normatividad ({valid.length} fuentes)
...
// ── Visor de evidencia con aprobación ───────────────────────────────
...
export default function ProjectDetail() {
  ...
  const filteredControls = controls.filter(c => {
    ...
    return (!q || c.code?.toLowerCase().includes(q) || c.causaBase?.toLowerCase().includes(q) || c.controlBase?.toLowerCase().includes(q))
      ...
      && (!filterDoc ? true : filterDoc==='doc' ? !!c.compliance?.trim() : !c.compliance?.trim());
  });
  ...
  const handleExport = async () => {
    setExporting(true);
    try { await exportMatriz(project); } catch(e) { alert('Error al exportar: '+e.message); } finally { setExporting(false); }
  };
    ...
    <>
      <div className="page-header">
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          <span className={`chip badge ${ROLE_COLORS[role]||'badge-gray'}`} style={{ fontSize:'.72rem' }}>{ROLES[role]?.l||role}</span>
          ...
          <button className="btn btn-secondary btn-sm" onClick={handleExport} disabled={exporting} title="Exportar XLSX + evidencias ZIP">
          ...
          {canFinalize && (
            <button
              ...
              onClick={() => { if (finalizeError) { alert(finalizeError); return; } setFinalModal(true); }}
              title={finalizeError || 'Finalizar y archivar esta matriz'}
              style={{ opacity: finalizeError ? .55 : 1 }}
            >
              Finalizar matriz
            </button>
          )}
      ...
      <div className="page-body">
        {/* Aviso de requisitos para finalizar */}
        {canFinalize && finalizeError && (
          <div className="alert alert-warning" style={{ marginBottom:16 }}>
            ...
            <span><strong>Para finalizar:</strong> {finalizeError}</span>
          </div>
        )}
          ...
          <>
            <div className="toolbar">
              ...
              <select className="form-control" style={{width:'auto'}} value={filterRisk} onChange={e=>setFilterRisk(e.target.value)}>
                ...
                <option value="Extremo">Extremo</option><option value="Alto">Alto</option><option value="Medio">Medio</option><option value="Bajo">Bajo</option>
              </select>
              <select className="form-control" style={{width:'auto'}} value={filterDoc} onChange={e=>setFilterDoc(e.target.value)}>
            ...
            <div className="table-wrap card">
              <table>
                <thead>
                  <tr>
                    <th>Código</th><th>Causa base / Causas asociadas</th><th>Control base</th>
                    <th>RI</th><th>RR</th>
                    <th>Cumplimiento {canFill&&<span style={{color:'var(--orange)',fontSize:'.65rem'}}>🎙️ Auto</span>}</th>
                    <th>Evidencias</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={c.id} style={{ background:c.compliance?.trim()?'rgba(34,197,94,.03)':'transparent' }}>
                        ...
                        <td style={{maxWidth:190}}>
                          ...
                          {c.causasAsociadas&&<div style={{fontSize:'.7rem',color:'var(--text-muted)',marginTop:2,lineHeight:1.3,whiteSpace:'pre-line'}}>{c.causasAsociadas.substring(0,100)}{c.causasAsociadas.length>100?'…':''}</div>}
                        </td>
                        <td style={{maxWidth:200}}>
                          <div style={{fontSize:'.8rem',lineHeight:1.4,whiteSpace:'pre-line'}}>{c.controlBase?.substring(0,140)}{c.controlBase?.length>140?'…':''}</div>
        ...
        {activeTab==='evidences' && (
          <>
            ...
            {filteredEvs.length===0
              ? <div className="empty card" style={{padding:60}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p>No hay evidencias{filterEvStatus?' con este estado':canAddEv?'. Ve a Controles para agregar.':'.'}</p></div>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8,fontSize:'.85rem',marginBottom:12}}>
                ...
                <div>✅ <strong>{evs.length}</strong> evidencias registradas ({evs.filter(e=>e.status==='aprobada').length} aprobadas)</div>
              </div>
